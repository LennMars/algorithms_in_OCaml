...
let rec binomial_generator m p ()=
  let rec binomial_generator_aux m accum =
    if m <= 0 then accum else
      if Random.float 1. < p then binomial_generator_aux (m - 1) (accum + 1) else binomial_generator_aux (m - 1) accum
  in
  binomial_generator_aux m 0;;

Random.init !seed;;

(* let _ = print_int (binomial 5 0.5 ()); print_newline () *)
...
let binomial_expectator m sequence current_value =
  ...
  in
  let normalizer = List.fold_left (+.) 0. (init_list max_likelihood_dist (m + 1))in
